...
def summarize(bars: Sequence[DailyBar]) -> dict[str, object]:
    ...
    return {
        ...
        "annual_return_best": max(annual_returns.values()) if annual_returns else None,
        "annual_return_worst": min(annual_returns.values()) if annual_returns else None,
        "current_year_ytd": annual_returns.get(str(bars[-1].trade_date.year)),
